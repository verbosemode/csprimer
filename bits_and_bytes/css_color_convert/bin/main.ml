...
let hexstr_to_rgb s =
  ...
  in
  let rec aux i si sj left_shift_i =
    if sj >= slen then
      i
    else (
      let i = i lxor (uint32_of_hex_string s si sj lsl left_shift_i) in
      aux i
        Int.(add si step_width)
        Int.(add sj step_width)
        Int.(sub left_shift_i 8)
    )
  in
  aux zero si sj 24
